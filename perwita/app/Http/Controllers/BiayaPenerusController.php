		    ...
		    $id_bpd = DB::table('biaya_penerus_dt')
		     ...
		     if($id_bpd == null){
		    	...
		    	for ($i=0; $i < count($request->seq_biaya); $i++) { 
		        	...
		        	$id_akun=[];
		foreach  ($request->kode_biaya as $index => $value) {
            $id_akun[$index]['acc_biaya'] = $value;
            $id_akun[$index]['total']      = $request->bayar_biaya[$index];
           }
...
$indexakun=0;
 ...
 $akunHutang=master_akun::
                  ...
                  ->where('id_akun','like', ''.$request->akun_agen.'%')                                    
        ...
        $akun[$indexakun]['value']=$total_tarif;
        $akun[$indexakun]['dk']='K';
        $indexakun++;      
        }
        else{
...
foreach ($dataJurnal as  $Nilaijurnal) {    
       $akunBiaya=master_akun::
                  select('id_akun','nama_akun')
                  ->where('id_akun','like', ''.$Nilaijurnal['acc_biaya'].'%')                                    
                  ->where('kode_cabang',$cabang)
                  ->orderBy('id_akun')
                  ->first();     
...
if($pending_status='APPROVED'){  
		...
		public function edit($id){
			...
			//	dd($nosppid);
				...
				// return $nota;
				if($cari_tipe[0]->fp_jenisbayar == 6 ){//biaya penerus hutang
					// return 'asd';
			 ...
			 $jurnal_dt=collect(\DB::select("SELECT id_akun,nama_akun,jd.jrdt_value,jd.jrdt_statusdk as dk
                        ...
                        (select j.jr_id from d_jurnal j where jr_ref='$id_jurnal')")); 							  
					...
					return view('purchase/fatkur_pembelian/editbiaya',compact('data',
																			  ...
																			  'vendor',
																			  'nota',
																			  'valid_cetak','jurnal_dt'));
			}else if($cari_tipe[0]->fp_jenisbayar == 7){//biaya penerus outlet
			 ...
			 $jurnal_dt=collect(\DB::select("SELECT id_akun,nama_akun,jd.jrdt_value,jd.jrdt_statusdk as dk
				...
				return view('purchase/fatkur_pembelian/editOutlet',compact('data','data_dt','valid_cetak','nota','tgl1','tgl2','agen','id','jurnal_dt'));
			}else if($cari_tipe[0]->fp_jenisbayar == 9){//biaya penerus subcon
					// return 'asd';
				...
				$jt = Carbon::parse($data->fp_jatuhtempo)->format('d/m/Y');
				$date = Carbon::now()->format('d/m/Y');
				...
				$pilih = DB::table('faktur_pembelian')
						  ...
						  ->where('fp_idfaktur','=',$id)
						  ->where('ksd_asal',(int)$data->pb_asal)
						  ->where('ksd_tujuan',(int)$data->pb_tujuan)
			...
			/*$id_jurnal=$data[0]->bp_faktur;
			 ...
			 $jurnal_dt=collect(\DB::select("SELECT id_akun,nama_akun,jd.jrdt_value,jd.jrdt_statusdk as dk
                        ...
                        (select j.jr_id from d_jurnal j where jr_ref='$id_jurnal')")); 	
*/
				return view('purchase/fatkur_pembelian/editsubcon',compact('valid_cetak','pilih','nota','jt','data','date','kota','subcon','akun_biaya','data_dt','angkutan','id'));
		...
		public function notaoutlet(){
			$year =Carbon::now()->format('y'); 
			$month =Carbon::now()->format('m'); 

			 $idfaktur =   fakturpembelian::where('fp_comp' , '001')
		...
		//	dd($nosppid);
			...
			$nota = 'FP' . $month . $year . '/' . '001' . '/O-' .  $idfaktur;
			/*dd($data['nofp']);*/

			return response()->json(['nota' => $nota]);

		}
		public function notapenerusagen(){
			$year =Carbon::now()->format('y'); 
			$month =Carbon::now()->format('m'); 
		...
		//	dd($nosppid);
			...
			$nota = 'FP' . $month . $year . '/' . '001' . '/P-' .  $idfaktur;
			/*dd($data['nofp']);*/

			return response()->json(['nota' => $nota]);
			
		}
		public function notasubcon(){
			$year =Carbon::now()->format('y'); 
			$month =Carbon::now()->format('m'); 
		...
		//	dd($nosppid);
			...
			$nota = 'FP' . $month . $year . '/' . '001' . '/SC-' .  $idfaktur;
			/*dd($data['nofp']);*/
		   ...
		   $cari_tt = DB::table('form_tt')
			...
			for ($i=0; $i < count($request->chck); $i++) { 
				if ($request->chck[$i] == 'on') {
		        	...
		        	$id_akun=[];
		foreach  ($request->comp as $index => $value) {									
			if ($request->chck[$index] == 'on') {
	            $id_akun[$index]['comp'] = $value;
	            $id_akun[$index]['total']      = $request->total_komisi[$index];
        	}
           }        

$dataJurnal=$this->groupJurnalOutlet($id_akun);
...
$indexakun=0;
 ...
 $akunHutang=master_akun::
        ...
        if(count($akunHutang)!=0){
        $akun[$indexakun]['id_akun']=$akunHutang->id_akun;
        $akun[$indexakun]['value']=filter_var($request->total_all_komisi, FILTER_SANITIZE_NUMBER_FLOAT);
        $akun[$indexakun]['dk']='K';
        $indexakun++;      
        }
        else{
...
foreach ($dataJurnal as  $Nilaijurnal) {    
       $akunBiaya=master_akun::
				  ...
				  ->where('kode_cabang','like', ''.$Nilaijurnal['comp'].'%')                                                         
                  ->orderBy('id_akun')
                  ->first();     
        ...
        $indexakun++;      
            ...
            d_jurnal::create([
                        ...
                        'jr_year'=> date('Y'),
                        'jr_date'=> date('Y-m-d'),
                        'jr_detail'=> 'BIAYA PENERUS Outlet',
                        'jr_ref'=> $request->nofaktur,
                        'jr_note'=> 'BIAYA PENERUS Outlet',
                        ]);
            d_jurnal_dt::insert($jurnal_dt);
    ...
    public function groupJurnalOutlet($data) {        
